...
const updatePinnedCard = (current) => {
  ...
  const arrivalEnd = Math.max(window.innerHeight * 2.8, aboutTop - window.innerHeight * .06);
  const journey = clamp(current / arrivalEnd);
  const smoothJourney = journey * journey * (3 - 2 * journey);
  ...
  const x = mix(0, targetCenterX - window.innerWidth * .5, smoothJourney);
  const y = mix(0, targetCenterY - baseTop, smoothJourney);
  const z = Math.sin(journey * Math.PI) * 3.5;
  const rotateY = current >= aboutTop ? 0 : Math.sin(journey * Math.PI) * 18;
  const panelHeight = aboutPanel?.offsetHeight || window.innerHeight;
  const approachStart = aboutTop - window.innerHeight * .55;
  const revealStart = aboutTop - window.innerHeight * .12;
  const approachEnd = revealStart;
  const approachFade = clamp((current - approachStart) / Math.max(1, approachEnd - approachStart));
  const arrival = clamp((current - revealStart) / Math.max(1, aboutTop - revealStart));
  const fadeStart = aboutTop + Math.min(panelHeight * .48, window.innerHeight * .5);
  const fadeEnd = aboutTop + Math.min(panelHeight * .78, window.innerHeight * .82);
  const departure = clamp((current - fadeStart) / Math.max(1, fadeEnd - fadeStart));
  const visibility = current < revealStart ? 1 - approachFade : current < aboutTop ? arrival : 1 - departure;
  const arrivalScale = current < revealStart ? 1 : mix(.86, 1, arrival);
  const departureScale = mix(1, .42, departure);
  pinnedCard.style.opacity = String(visibility);
  pinnedCard.style.transform = `translate(-50%,-50%) translate3d(${x}px,${y}px,0) rotateZ(${z}deg) scale(${arrivalScale * departureScale})`;
